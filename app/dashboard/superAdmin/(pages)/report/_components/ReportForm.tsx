...
const ReportForm: React.FC<ReportFormProps> = ({ onReportGenerated }) => {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        // Fetch products
        const productsResponse = await fetch(`${API}/superAdmin/product`);
        ...
        // Fetch categories
        const categoriesResponse = await fetch(`${API}/superAdmin/category`);
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API}/superAdmin/report?${params.toString()}`);
