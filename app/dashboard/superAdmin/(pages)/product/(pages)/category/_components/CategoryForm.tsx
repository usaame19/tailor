...
const CategoryForm = ({ category }: { category?: Category }) => {
  ...
  const onSubmit = async (values: z.infer<typeof categorySchema>) => {
    try {
      if (category) {
        await axios.patch(`${API}/superAdmin/category/${category.id}`, values);
      } else {
        await axios.post(`${API}/superAdmin/category`, values);
      }
      ...
      router.push("/dashboard/superAdmin/product/category");
    } catch (err) {
