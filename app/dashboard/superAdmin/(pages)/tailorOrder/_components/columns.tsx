    ...
    cell: ({ row }) => {
      ...
      const handleDelete = async () => {
        if (window.confirm("Are you sure you want to delete this tailor order?")) {
          try {
            await axios.delete(`${API}/superAdmin/tailorOrder/${tailorOrder.id}`);
      ...
      return (
        <div className="flex space-x-2">
          <Link href={`/dashboard/superAdmin/tailorOrder/${tailorOrder.id}`}>
            <Button variant="outline" size="sm">
              <FaEye className="h-4 w-4" />
            </Button>
          </Link>
          <Link href={`/dashboard/superAdmin/tailorOrder/${tailorOrder.id}/edit`}>
            <Button variant="outline" size="sm">
              <FaEdit className="h-4 w-4" />
            </Button>
          </Link>
          <Link href={`/dashboard/superAdmin/tailorOrder/${tailorOrder.id}/payment`}>
