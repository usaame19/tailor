      ...
      if (watchCategory) {
        try {
          const response = await axios.get(
            `${API}/superAdmin/transaction/category/${watchCategory}`
          );
  ...
  const onSubmit = async (values: z.infer<typeof transactionSchema>) => {
    ...
    try {
      const response = transaction
        ? await axios.patch(`${API}/superAdmin/transaction/${transaction.id}`, {
          ...values,
          tranDate: formattedDate, // Ensure date is correctly formatted
        })
        : await axios.post(`${API}/superAdmin/transaction`, {
          ...values,
          tranDate: formattedDate, // Ensure date is correctly formatted
        });
      ...
      router.push("/dashboard/superAdmin/transaction");
    } catch (error) {
