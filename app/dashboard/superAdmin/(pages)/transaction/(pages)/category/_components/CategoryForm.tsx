  ...
  const onSubmit = async (
    ...
    try {
      ...
      if (transactionCategory) {
        await axios.patch(
          `${API}/superAdmin/transaction/category/${transactionCategory.id}`,
          updatedValues
        );
      } else {
        await axios.post(`${API}/superAdmin/transaction/category`, updatedValues);
      }
      ...
      router.push("/dashboard/superAdmin/transaction/category");
    } catch (err) {
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              {/* superAdmin-only toggle */}
              <FormField
                control={form.control}
                name="isAdmin"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-gray-700">superAdmin Only</FormLabel>
                    <FormControl>
                      <Checkbox
                      ref={(e) => {
                        field.ref(e);
