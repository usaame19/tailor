...
export default function AddCustomerSizeForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "new") {
        const response = await axios.post(`${API}/admin/customer`, { customer, size });
        ...
        const response = await axios.post(`${API}/admin/customer/${selectedCustomerId}/size`, { size });
        if (response.status === 200) {
          toast.success("Measurements added to existing customer successfully!");
        }
      }
      router.push("/dashboard/admin/customer");
    } catch (error) {
