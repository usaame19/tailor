...
const TailorOrderForm = ({ order }: { order?: TailorOrder }) => {
  ...
  const { data: customers, isLoading: customersLoading } = useQuery({
    queryKey: ["customers"],
    queryFn: () => axios.get(`${API}/admin/customer`).then((res) => res.data),
  });
  ...
  const onSubmit = async (data: any) => {
    try {
      if (order) {
        ...
        await axios.patch(`${API}/admin/tailorOrder/${order.id}`, data);
        toast.success("Tailor order updated successfully");
        router.push(`/dashboard/admin/tailorOrder/${order.id}`);
      } else {
        // Create new order
        await axios.post(`${API}/admin/tailorOrder`, data);
        toast.success("Tailor order created successfully");
        router.push("/dashboard/admin/tailorOrder");
      }
    } catch (error: any) {
